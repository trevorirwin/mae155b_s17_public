...
% Load data
%
load('rp1-counter-v2.mat');
load('rp1-analog-v2.mat')
%
% Offset T = 0 to start of data.
%
test_offset = 256.9; % [s] Time at which test started
...
title('Chamber Pressure vs Time (RP1)','fontsize',fsbig);
...
title('Flow Rate vs Time (RP1)','fontsize',fsbig);
lgd = legend('Fuel', 'LOX');
lgd.FontSize = fsmed;
set(gca, 'FontSize',fsmed)
%
...
rho_rp1 = 840;              % [kg/m^3] Assume constant density rp1
...
q_dot_rp1 = cdata_long(:,9)*6.30902e-5; % [gal/min -> m^3/s]
q_dot_lox = cdata_long(:,10)*6.30902e-5; % [gal/min -> m^3/s]

m_dot_rp1 = q_dot_rp1.*rho_rp1; % [kg/s]
m_dot_lox = q_dot_lox.*rho_lox; % [kg/s]

m_dot_t = m_dot_rp1 + m_dot_lox; % [kg/s]
...
title('Thrust vs Time (RP1)','fontsize',fsbig);
set(gca, 'FontSize',fsmed)
...
title('Specific Impulse vs Time (RP1)','fontsize',fsbig);
set(gca, 'FontSize',fsmed)
...
xlabel('Time [s]','fontsize',fsbig);
ylabel('Thrust [lbf]','fontsize',fsbig);
title('Thrust & Flow Rate vs Time (RP1)','fontsize',fsbig);
yyaxis right;
