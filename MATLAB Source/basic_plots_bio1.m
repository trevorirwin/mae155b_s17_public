...
% Load data
%
load('bio-test1-counter-v1.mat');
load('bio-test1-analog-v1.mat')
adata = adata_bio1;
cdata = cdata_bio1;
%
% Offset T = 0 to start of data.
%
test_offset = 89.31; % [s] Time at which test started
...
title('Chamber Pressure vs Time (Biodiesel Test 1)','fontsize',fsbig);
...
title('Flow Rate vs Time (Biodiesel Test 1)','fontsize',fsbig);
lgd = legend('Fuel', 'LOX');
lgd.FontSize = fsmed;
set(gca, 'FontSize',fsmed)
%
...
title('Thrust vs Time (Biodiesel Test 1)','fontsize',fsbig);
set(gca, 'FontSize',fsmed)
...
title('Specific Impulse vs Time (Biodiesel Test 1)','fontsize',fsbig);
set(gca, 'FontSize',fsmed)
...
xlabel('Time [s]','fontsize',fsbig);
ylabel('Thrust [lbf]','fontsize',fsbig);
title('Thrust & Flow Rate vs Time (Biodiesel Test 1)','fontsize',fsbig);
yyaxis right;
